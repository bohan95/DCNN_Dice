...
ROI_EMBEDDING_DIM = 16
NUM_ROI_CLASSES = 693

# 定义 ROI Embedding 层
roi_embedding_layer = nn.Embedding(NUM_ROI_CLASSES, ROI_EMBEDDING_DIM)
...
def preprocess_fiber_input(data):
    ...
    - 3D 坐标 (b, 3, 100)
    - ROI 分类索引 (b, 1, 100)
    - 使用 nn.Embedding 将 ROI 转换为连续特征 (b, 32, 100)
    - 拼接得到最终输入 (b, 3+32, 100)

    参数:
        data: Tensor, shape (b, 4, 100)，其中
            - data[:, :3, :] 是 3D 坐标
            - data[:, 3, :] 是 ROI 分类索引（整数）

    返回:
        processed_data: Tensor, shape (b, 3+32, 100)
    """
    # 获取 3D 坐标
    coord_3d = data[:, :3, :]  # (b, 3, 100)

    # 获取 ROI 分类索引，并转换为 long 类型
    roi_indices = data[:, 3, :].long()  # (b, 100)

    # 通过 Embedding 层获取 ROI 的嵌入特征 (b, 100, 32)
    roi_features = roi_embedding_layer(roi_indices)  # (b, 100, 32)

    # 变换维度，使其与 3D 坐标匹配 (b, 32, 100)
    roi_features = roi_features.permute(0, 2, 1)

    # 拼接 3D 坐标和 ROI 特征，作为模型输入 (b, 3+32, 100)
