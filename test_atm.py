...
def loadmat(filename):
    ...
    try:
        ...
        output=dict()
        data = h5py.File(filename, mode='r') # error corrected by MH 10/12/2022 (add mode='r') 
        count=data['tracks']['count'][()] # error corrected by MH 10/12/2022 (delete .value and add [()])
        total_count=0
        for i in count:
            total_count*=10
...
def argvHelp():
    ...
    print('python scriptname.py /path/to/mat /path/to/model classnum')
def checkArgc(argv):
    if(len(argv)!=4):
        argvHelp()
