...
def initialize_cluster_rois_with_kmeans(X_train, num_clusters, num_anatomical_rois):
    """
    用 K-Means 初始化 global_cluster_rois
    """
    print("Running K-Means to initialize global_cluster_rois...")

    # 取部分数据进行 K-Means
    sample_size = min(100000, X_train.shape[0])  # 选取最多 10 万条数据
    sample_data = X_train[:sample_size, 0:3, :].reshape(sample_size, -1).cpu().numpy()  # 只取 3D 坐标

    # 运行 K-Means
    kmeans = KMeans(n_clusters=num_clusters, random_state=42, n_init=10)
    cluster_labels = kmeans.fit_predict(sample_data)

    # 计算每个 cluster 的 anatomical profile
    fiber_rois_sample = X_train[:sample_size, -1, :]  # 获取 ROI 信息
    cluster_rois = compute_cluster_anatomical_profile(
        torch.tensor(cluster_labels, device=X_train.device), fiber_rois_sample, num_clusters, num_anatomical_rois
    )

    print("K-Means initialization completed!")
    print(f"cluster_rois: {cluster_rois[0]}")
    return cluster_rois
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(trn_loader):
        ...
        if args.use_clustering_loss:
            ...
            # updating global cluster rois
            if args.use_dice_a_loss:
                pred_clusters = clustering_out.argmax(dim=1)
                # calculate batch level cluster ROI profile
                batch_cluster_rois = compute_cluster_anatomical_profile(
                    pred_clusters, data[:, -1, :], NCLASS, num_anatomical_rois
                )
                # 记录更新前的 global_cluster_rois
                before_update = global_cluster_rois.clone().detach()

                # 更新 global_cluster_rois
                momentum = 0.6  # 降低动量
                global_cluster_rois = momentum * global_cluster_rois + (1 - momentum) * batch_cluster_rois

                # 只保留 top-k
                top_k = 10  # 增加 top-k
                _, top_indices = torch.topk(global_cluster_rois, top_k, dim=1)
                global_cluster_rois = torch.zeros_like(global_cluster_rois)
                global_cluster_rois.scatter_(1, top_indices, 1)

                # 计算变化程度
                change = (global_cluster_rois - before_update).abs().sum().item()
                if change > 0.0:
                    print(f'Cluster ROIs 更新变化: {change}')
...
if args.use_dice_a_loss:
    global_cluster_rois = initialize_cluster_rois_with_kmeans(X_train, NCLASS, num_anatomical_rois)
